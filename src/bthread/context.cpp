 ...
 *  ----------------------------------------------------------------------------------  *
 *  |        R15        |        RBX        |         RBP        | (bthread_make_fcontext第3个参数，intptr_t)         |  *
 *  ----------------------------------------------------------------------------------  *
 *  ----------------------------------------------------------------------------------  *
 *  |    16    |    17   |                                                              *
 *  ----------------------------------------------------------------------------------  *
 ...
 *  ----------------------------------------------------------------------------------  *
 *  ----------------------------------------------------------------------------------  *
 *  通过被调用方保存寄存器，栈是从高往低生长
 ****************************************************************************************/
"    pushq  %rbp  \n" /*pushq指令的行为等价于：subq $8, %rsp 与movq REG, (%rsp)两条指令的合效果*/
"    pushq  %rbx  \n"
"    pushq  %r15  \n"
"    pushq  %r14  \n"
"    pushq  %r13  \n"
"    pushq  %r12  \n"
"    leaq  -0x8(%rsp), %rsp\n"
"    cmp  $0, %rcx\n" /*比较第4个参数 preserve_fpu，是否保存 */
"    je  1f\n"
"    stmxcsr  (%rsp)\n"
"    fnstcw   0x4(%rsp)\n"
"1:\n"
"    movq  %rsp, (%rdi)\n" /*将当前栈顶存入第一参数&from->context*/
"    movq  %rsi, %rsp\n" /*将第二个参数to->context存入新的栈顶中*/
"    cmp  $0, %rcx\n" /*比较第4个参数 preserve_fpu，是否保存 preserve_fpu*/
"    je  2f\n"
"    ldmxcsr  (%rsp)\n"
"    fldcw  0x4(%rsp)\n"
"2:\n"
...
"    popq  %rbx  \n"
"    popq  %rbp  \n"
"    popq  %r8\n" /*intptr_t中参数是 intptr_t，即一个函数地址*/
"    movq  %rdx, %rax\n" /*将第3个参数intptr_t存入%rax，%rax是函数返回值*/
"    movq  %rdx, %rdi\n" /*将第3个参数intptr_t存入%rdi，作为跳转 intptr_t 的第一个参数, intptr_t 即 task_runner */
"    jmp  *%r8\n"
".size bthread_jump_fcontext,.-bthread_jump_fcontext\n"
".section .note.GNU-stack,\"\",%progbits\n"
);
...
".align 16\n"
"bthread_make_fcontext:\n"
"    movq  %rdi, %rax\n" /* bthread_make_fcontext 的第一个参数加载到rax（即栈底）*/
/* 先对栈指针进行16字节对齐
     *
     *
  低---------------------------------------------------|----高
     ...
     * 先对栈指针进行16字节对齐
     */
"    andq  $-16, %rax\n" /* andq S, D D = D & S, 将context-stack 调整到向下16字节对齐，此时RAX指向真正的context-stack可以使用的基地址*/
"    leaq  -0x48(%rax), %rax\n" /* 保留72字节的地址留给context */
"    movq  %rdx, 0x38(%rax)\n" /* 将当前第三个参数，即fn函数路口，放入栈中）*/
"    stmxcsr  (%rax)\n" /* 将fc_mxcsr 存入栈中，存储MXCSR寄存器状态 */
"    fnstcw   0x4(%rax)\n" /*将fc_x87_cw存入栈中*/
"    leaq  finish(%rip), %rcx\n" /*计算finish绝对地址，并且保存在RIP中，后续调用bthread_jump_fcontext的时候首先会进入调用这个函数*/
"    movq  %rcx, 0x40(%rax)\n" /*将函数地址压栈*/
"    ret \n"  /*返回 指向context-data的，0x0处，返回rax当前值（rax是返回值寄存器）*/
"finish:\n"
"    xorq  %rdi, %rdi\n" /*它实现了对%rdi的置0，然后调用exit(0)，相当于传入0*/
"    call  _exit@PLT\n"
"    hlt\n"
".size bthread_make_fcontext,.-bthread_make_fcontext\n"
".section .note.GNU-stack,\"\",%progbits\n"
